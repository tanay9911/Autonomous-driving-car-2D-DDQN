...
# Defining reward constants
GOALREWARD = 1    # Reward for reaching a goal
LIFE_REWARD = 0   # Reward for staying alive
PENALTY = -1      # Penalty for crashing into walls

# Defining function to calculate distance between two points
def distance(pt1, pt2):
    return(((pt1.x - pt2.x)**2 + (pt1.y - pt2.y)**2)**0.5)

# Defining function to rotate a point around an origin by given angle
...
# Defining function to rotate rectangle points by given angle
...
# Defining class to represent a point
class myPoint:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        
# Defining class to represent a line segment
class myLine:
    def __init__(self, pt1, pt2):
        self.pt1 = myPoint(pt1.x, pt1.y)
        self.pt2 = myPoint(pt2.x, pt2.y)

# Defining class to represent a ray for collision detection
class Ray:
    ...
    # Defining method to cast ray and check for wall intersection
    def cast(self, wall):
        x1 = wall.x1 
        y1 = wall.y1
        x2 = wall.x2
...
# Defining main Car class
class Car:
    def __init__(self, x, y):
        # Initializing car position and dimensions
        self.pt = myPoint(x, y)
        self.x = x
        self.y = y
        self.width = 14
        ...
        # Loading car image using relative path
        current_dir = os.path.dirname(__file__)
        car_image_path = os.path.join(current_dir, "car.png")
        self.original_image = pygame.image.load(car_image_path).convert()
        self.image = self.original_image  # This will reference the rotated image.
        self.image.set_colorkey((0,0,0))
        self.rect = self.image.get_rect().move(self.x, self.y)

        # Initializing car angle and velocity parameters
    ...
    # Defining method to handle car actions based on input choice
    ...
    # Defining method to handle acceleration
    ...
    # Defining method to handle turning
    def turn(self, dir):
        self.soll_angle = self.soll_angle + dir * math.radians(15)
    
    # Defining method to update car position and rotation
    def update(self):
    ...
    # Defining method to cast rays for collision detection
    def cast(self, walls):
        # Creating rays at different angles from car
        ...
        # Checking each ray for wall collisions
    ...
    # Defining method to check for wall collisions
    ...
    # Defining method to check for goal scoring
    def score(self, goal):
    ...
    # Defining method to reset car to initial state
    def reset(self):
        self.x = 50
        self.y = 300
        self.velX = 0
    ...
    # Defining method to draw car on screen
    def draw(self, win):
        win.blit(self.image, self.rect)
  

# Defining main environment class
class RacingEnv:
    def __init__(self):
        ...
        # Loading track image using relative path
        current_dir = os.path.dirname(__file__)
        track_image_path = os.path.join(current_dir, "track.png")
        self.back_image = pygame.image.load(track_image_path).convert()
    ...
    # Defining method to reset environment
    def reset(self):
        self.screen.fill((0, 0, 0))
    ...
    # Defining method to execute one game step
    def step(self, action):
        ...
        # Checking if car passes Goal and scores
        index = 1
        ...
        # Checking if car crashed into wall
    ...
    # Defining method to render game state
    def render(self, action):
        ...
        # Rendering control visualization
        ...
        text_surface = self.font.render(f'Points {self.car.points}', True, pygame.Color('darkblue'))
        self.screen.blit(text_surface, dest=(0, 0))
        # Rendering speed display
        text_surface = self.font.render(f'Speed {self.car.vel*-1}', True, pygame.Color('darkblue'))
        self.screen.blit(text_surface, dest=(800, 0))

        self.clock.tick(self.fps)
        pygame.display.update()

    # Defining method to close environment
    def close(self):
        pygame.quit()